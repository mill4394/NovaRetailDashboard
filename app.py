...
top_customers["CustomerLabel"] = "#" + top_customers["CustomerID"].astype(str)
...
fig_top_customers.update_layout(
    ...
    yaxis=dict(
        type="category",
        categoryorder="array",
        categoryarray=top_customers["CustomerLabel"].tolist(),
    ),
)
...
customer_summary_display = customer_summary.rename(
    columns={
        "CustomerID": "Customer ID",
        "TotalRevenue": "Total Revenue",
        "DistinctTransactions": "Transactions",
...
styled_customer_summary = (
    customer_summary_display.style
    .format(
        {
            "Total Revenue": "${:,.2f}",
            "Transactions": "{:,.0f}",
...
st.caption(
    "A segment is marked as needing attention when both its average "
    "satisfaction and revenue per customer fall below the filtered "
    "segment medians."
)

segment_summary_display = segment_summary.rename(
    columns={
        "label": "Behavioral Segment",
        "TotalRevenue": "Total Revenue",
...
segment_summary_display["Assessment"] = segment_summary_display[
    "Assessment"
].replace(
    {
        "Potentially At Risk": "⚠️ Needs Attention",
        "Monitor / Stable": "✓ Healthy",
    }
)


def highlight_at_risk(row):
    """Highlight rows that need management attention."""
    is_at_risk = "Needs Attention" in str(row["Assessment"])
    if is_at_risk:
        return [
...
styled_segment_summary = (
    segment_summary_display.style
    .apply(highlight_at_risk, axis=1)
    .format(
        {
            "Total Revenue": "${:,.2f}",
            "Unique Customers": "{:,.0f}",
...
with growth_chart_1:
    ...
    format_chart(fig_category, height=500)
...
daily_revenue = (
    filtered_df.assign(
        RevenueDate=filtered_df["TransactionDate"].dt.normalize()
    )
    .groupby("RevenueDate", as_index=False)["PurchaseAmount"]
    .sum()
    .sort_values("RevenueDate")
)
...
format_chart(fig_daily, height=400)
